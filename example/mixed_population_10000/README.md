# Mixed Population 10000

This example demonstrates a YSimulator configuration with:
- **10001 agents total**: **5000 LLM-enabled agents** and **5000 rule-based agents** and **1 news page**
- **Initial random social network** with ~100000 follow relationships (~10 connections per agent)
...
- **5000 LLM Agents**: AI-powered decision making using language models
- **5000 Rule-based Agents**: Heuristic-based decision making
...
```bash
cd example/mixed_population_10000
...
python run_server.py --config example/mixed_population_10000/server_config.json
```
...
python run_client.py --config example/mixed_population_10000/simulation_config.json \
                     --agents example/mixed_population_10000/agent_population.json \
                     --prompts example/mixed_population_10000/llm_prompts.json
```

## Configuration Files

### agent_population.json
Defines all 10001 agents with diverse characteristics:
...
### network.csv
Initial social network with approximately 100000 directed follow edges using Erdős–Rényi random graph model.
...
- **Network loading**: Expect ~100 seconds for initial network loading
- **LLM requirements**: 5000 LLM agents will make API calls to Ollama/OpenAI
- **Memory usage**: ~5000MB for agent population
