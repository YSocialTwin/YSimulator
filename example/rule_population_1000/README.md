...
- **1001 agents total**: **1000 rule-based agents** and **1 news page**
- **Initial random social network** with ~10000 follow relationships (~10 connections per agent)
...
- **1000 Rule-based Agents**: Heuristic-based decision making
...
```bash
cd example/rule_population_1000
...
python run_server.py --config example/rule_population_1000/server_config.json
```
...
python run_client.py --config example/rule_population_1000/simulation_config.json \
                     --agents example/rule_population_1000/agent_population.json \
                     --prompts example/rule_population_1000/prompts.json
```
...
### network.csv
Initial social network with approximately 10000 directed follow edges using Erdős–Rényi random graph model.
...
- **Network loading**: Expect ~10 seconds for initial network loading
- **LLM requirements**: 0 LLM agents will make API calls to Ollama/OpenAI
- **Memory usage**: ~500MB for agent population
