...
class InterestManager:
    ...
    def validate_and_extract_interests(self, interests) -> Tuple[Optional[List[str]], Optional[List[int]]]:
        """
    ...
    def recompute_agent_interests_from_window(self, agent_id: str):
        ...
        if not self.current_round_id:
            self.logger.warning(f"Cannot recompute interests for agent {agent_id}: current_round_id not set")
            return
    ...
    def set_agent_interests(self, agent_id: str, topics: List[str], counts: List[int]):
        ...
        if topics and counts:
            self.agent_interests[str(agent_id)] = {
                "topics": list(topics),
                "counts": list(counts)
            }
