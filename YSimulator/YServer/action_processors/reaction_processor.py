...
class ReactionProcessor(BaseActionProcessor):
    ...
    def process(self, action: Any, context: ActionContext) -> ActionResult:
        ...
        try:
            ...
            if success:
                count_updated = self.services.post_service.increment_post_reaction_count(
                    action.target_post_id
                )
                if count_updated:
                    self.logger.info(
                        f"Incremented reaction count for post {
                            action.target_post_id} after {
                            action.action_type} by agent {
                            action.agent_id}"
                    )
                else:
                    self.logger.warning(
    ...
    def _save_reaction_sentiment(self, action: Any, context: ActionContext) -> None:
        ...
        if not topic_ids:
            self.logger.debug(
                f"No topics found for reacted post {
                    action.target_post_id}, skipping reaction sentiment"
            )
            return
        ...
        for topic_id in topic_ids:
            ...
            if success:
                self.logger.info(
                    f"Added reaction sentiment for {
                        action.action_type} on post {
                        action.target_post_id}, topic {topic_id}"
                )
            else:
                self.logger.error(
                    f"Failed to add reaction sentiment for {
                        action.action_type} on post {
                        action.target_post_id}, topic {topic_id}"
                )
    ...
    def _get_parent_sentiment(self, post_id: str) -> str:
        ...
        if parent_sentiment_data is not None:
            ...
            if sentiment_parent_compound is not None:
                ...
                self.logger.info(
                    f"Parent sentiment for reaction on post {post_id}: compound={
