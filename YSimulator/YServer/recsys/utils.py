...
def fetch_common_interest_posts(
    ...
    base_query = (
        db.session.query(Post, func.count(PostTopic.topic_id).label("match_count"))
...
def fetch_common_user_interest_posts(
    ...
    user_interests = db.session.query(UserInterest.interest_id).filter_by(user_id=uid)
    common_users = (
        db.session.query(
            User_mgmt.id.label("user_id"),
            func.count(UserInterest.interest_id).label("match_count"),
        )
        .join(UserInterest, User_mgmt.id == UserInterest.user_id)
        .filter(User_mgmt.id != uid, UserInterest.interest_id.in_(user_interests))
...
def get_posts_by_reactions(
    ...
    posts = (
        db.session.query(Post, func.count(Reaction.user_id).label("total"))
        .join(Reaction, Post.id == Reaction.post_id)
        .filter(
            Reaction.user_id.in_(user_ids),
            Reaction.type.in_(reactions_type),
            Post.round >= visibility,
            Post.news_id != -1 if articles else True,
        )
        .group_by(Post.id)
