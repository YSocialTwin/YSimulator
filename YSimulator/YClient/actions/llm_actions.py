...
def generate_image_post_async(server, llm_service, agent_cluster: int, image_data: dict, 
                               topics: List[str] = None, agent_attrs: dict = None):
    """
    Generate an async LLM-based image post with commentary.
    
    This function creates a post that shares an image with personalized commentary
    generated by the LLM based on the image description and related topics.
    
    Args:
        server: Ray actor reference for the server (to save post)
        llm_service: Ray actor reference for LLMService
        agent_cluster: Agent's cluster ID for persona
        image_data: Dict with keys: id, url, description, article_id
    ...
    Returns:
        tuple: (Ray ObjectRef for commentary future, image_id)
        
    Usage:
        image_data = ray.get(server.get_random_image.remote())
        topics = ray.get(server.get_article_topics.remote(image_data['article_id']))
        future, image_id = generate_image_post_async(server, llm, cluster, image_data, topics, attrs)
        commentary = ray.get(future)
        # Create ActionDTO with image_id
    """
    # Get image description and topics
    image_description = image_data.get("description", "An image")
    image_id = image_data.get("id")
    
    # Call LLM service to generate commentary
    commentary_future = llm_service.generate_image_commentary.remote(
        image_description, topics, agent_attrs, agent_cluster
    )
    
    return commentary_future, image_id
