...
class ReplyGenerator(BaseActionGenerator):
    ...
    def generate(self, agent: AgentProfile, agent_type: str) -> ActionGeneratorResult:
        ...
        try:
            ...
            self.context.logger.info(
                f"[REPLY] Agent {
                    agent.username} ({agent_type}) selected mention {mention_id} in post {post_id}"
            )
            ...
            if agent_type == "llm":
                ...
                self.context.logger.debug(
                    f"[REPLY] Retrieved thread context: {
                        len(thread_context)} previous posts/comments"
                )
                ...
                self.context.logger.info(
                    f"[REPLY] LLM reply request queued for agent {
                        agent.username} (mention: {mention_id})"
                )
            else:
