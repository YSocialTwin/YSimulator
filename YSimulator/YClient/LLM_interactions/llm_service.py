...
class LLMService:
    ...
    def generate_read_reaction(
        ...
        user_msg = self._render_user_template(
            user_template,
            {"persona": persona, "post_content": post_content},
            agent_attrs=agent_attrs,
            memory_header="CONVERSATION MEMORY",
        )
        ...
        try:
            ...
            if "ANGRY" in result or "DISLIKE" in result:
                return "ANGRY"  # Map DISLIKE to ANGRY
            if "SAD" in result:
                return "SAD"
            if "IGNORE" in result:
                return "IGNORE"
    ...
    def describe_image(self, image_url: str) -> Optional[str]:
        ...
        describe_image_config = self.prompts_config.get(
            "describe_image", DEFAULT_IMAGE_DESCRIPTION_PROMPTS
        )
        system_template = describe_image_config.get(
            "system_template", DEFAULT_IMAGE_DESCRIPTION_PROMPTS["system_template"]
        )
        user_template = describe_image_config.get(
            "user_template", DEFAULT_IMAGE_DESCRIPTION_PROMPTS["user_template"]
        )
    ...
    def evaluate_opinion(
        ...
        prompt = ChatPromptTemplate.from_messages(
            [("system", system_template), ("user", prompt_text)]
        )
