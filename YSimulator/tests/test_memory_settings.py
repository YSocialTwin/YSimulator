...
def test_memory_enabled_defaults_to_native_backend():
...
def test_memory_enabled_accepts_valid_backends(backend):
    settings = resolve_memory_settings({"agent_memory": {"enabled": True, "backend": backend}})
    assert settings.enabled is True
...
def test_memory_enabled_rejects_invalid_backend():
    with pytest.raises(ValueError, match="Invalid memory backend"):
        resolve_memory_settings({"agent_memory": {"enabled": True, "backend": "invalid_backend"}})
