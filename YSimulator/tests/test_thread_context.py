...
class TestThreadContext(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with Session(self.db.engine) as session:
            ...
            base_day = (
                TestThreadContext._round_counter * 10
            )  # Use counter-based day to ensure uniqueness
            ...
            user1 = User_mgmt(
                id=self.user1_id, username=f"alice_{unique_id}", password="test_password"
            )
            user2 = User_mgmt(
                id=self.user2_id, username=f"bob_{unique_id}", password="test_password"
            )
            user3 = User_mgmt(
                id=self.user3_id, username=f"charlie_{unique_id}", password="test_password"
            )
