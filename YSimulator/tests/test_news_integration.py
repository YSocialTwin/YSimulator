...
class TestNewsIntegration(unittest.TestCase):
    ...
    def test_01_website_model_structure(self):
        ...
        self.assertEqual(
            str(id_col.type), "VARCHAR(36)", "Website.id should be VARCHAR(36) for UUID"
        )
    ...
    def test_02_article_model_structure(self):
        ...
        self.assertEqual(
            str(fk.column.table), "websites", "Article.website_id should reference websites table"
        )
    ...
    def test_07_duplicate_website_prevention(self):
        ...
        self.assertEqual(
            website_id_1, website_id_2, "Adding same RSS URL should return existing website_id"
        )
        ...
        with Session(self.db.engine) as session:
            stmt = (
                select(func.count())
                .select_from(Website)
                .where(Website.rss == "https://test.com/feed.xml")
            )
